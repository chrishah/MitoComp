rule get_organelle:
    ...
    params:
        ...
        seed = get_seed,
	non_default = config["Assembler_options"]["getorganelle"]["non_default"],
        type = get_type,
        rounds = get_rounds
    ...
    shell:
        ...
        get_organelle_from_reads.py -1 {params.wd}/{input.f} -2 {params.wd}/{input.r} -o {params.wd}/{params.outdir}/run -F {params.type} -t {threads} -R {params.rounds} -s {params.wd}/{params.seed} {params.non_default} 1> {params.wd}/{log.stdout} 2> {params.wd}/{log.stderr} && returncode=$? || returncode=$?
        if [ $returncode -gt 0 ]
        then
        ...
        then #check first of folder exists in case get_organelle exits with the wrong exit code.
            final_fasta=$(find {params.wd}/{params.outdir}/run -maxdepth 1 -name "*.fasta")
        fi
        ...
        then
            ...
            ln -sf $final_fasta {wildcards.id}.{wildcards.sub}.getorganelle.fasta
