rule trimgalore:
        ...
        log:
                stdout = "output/{id}/logs/trimgalore.stdout.txt",
                stderr = "output/{id}/logs/trimgalore.stderr.txt"
        output:
...
rule trimmomatic:
    ...
    log:
        stdout = "output/{id}/logs/trimmomatic.stdout.txt",
        stderr = "output/{id}/logs/trimmomatic.stderr.txt"
    threads: config["threads"]["trimmomatic"] 
    singularity: 
    ...
    shell:
        ...
        if [[ ! -f {params.adapter} ]]; then
            echo "Adpater file not found. Please check your config files." >&2 {log.stderr}
            exit 1
        fi
        trimmomatic PE -threads {threads} {input.f} {input.r} {output.fout} {output.funp} {output.rout} {output.runp} ILLUMINACLIP:{params.adapter}:{params.seed_mismatches}:{params.palindrome_clip}:{params.simple_clip} LEADING:{params.quality} TRAILING:{params.quality} SLIDINGWINDOW:{params.windowsize}:{params.required_quality} MINLEN:{params.minlength} 1> {log.stdout} 2>> {log.stderr}
        touch {output.ok}
        """
